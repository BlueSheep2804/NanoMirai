...
class NanoSwarmBlasterItem(override val tier: NanoTier) : PoweredItem(
    Properties().rarity(tier.rarity).component(DataComponents.POTION_CONTENTS, PotionContents.EMPTY),
    10000,
    1000
), INanoTieredItem {
    companion object {
        fun addEffect(stack: ItemStack, effect: MobEffectInstance) {
            if (!stack.`is`(NanoMiraiItemTags.NANO_SWARM_BLASTER)) return
    ...
    override fun releaseUsing(stack: ItemStack, level: Level, livingEntity: LivingEntity, timeCharged: Int) {
        ...
        if (stack.damageValue < stack.maxDamage - 1) {
            ...
            livingEntity.cooldowns.addCooldown(this, tier.blasterCooldown)
        }
    }
